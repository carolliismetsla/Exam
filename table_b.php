...
<?php
	...
	*/
	if(isset($_GET["print"])){
		...
		echo "printing row with id:".$_GET["print"];

		// NOW() = current date-time
		$stmt = $mysql->prepare("UPDATE exam SET printed=NOW() WHERE id = ?");

		echo $mysql->error;

		//replace the ?
		$stmt->bind_param("i", $_GET["print"]);
	...
	//SQL sentence
	$stmt = $mysql->prepare("SELECT id, document_name, pages, color_or_bw  FROM exam WHERE printed IS NULL ORDER BY created DESC ");
